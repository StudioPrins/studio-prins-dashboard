    ...
    <section className="card p-6">
      ...
      <ul className="flex flex-col">
        ...
        {fields.map((f, i) => (
          <li
            key={f.id}
            className="flex flex-wrap items-start gap-2 py-2.5 border-b border-line last:border-0"
          >
            <span className="text-xs text-muted w-5 shrink-0 tabular-nums pt-1.5">{i + 1}.</span>

            {/* Ondergrens zodat de vraag leesbaar blijft: past de rest er niet
                naast, dan wipt die naar de volgende regel in plaats van de tekst
                tot een paar pixels te knijpen. */}
      ...
      <form onSubmit={onAdd} className="mt-4 flex flex-wrap gap-2">
        <input ref={labelRef} className="input flex-1 min-w-[11rem]" placeholder="Vraag toevoegen…" />
